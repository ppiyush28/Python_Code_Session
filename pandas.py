...
users = pd.read_table('Data/u.user', header=None, sep='|', names=u_cols, index_col='user_id',
                      dtype={'zip_code':str,age': int})
...
users[users.occupation.isin(['doctor', 'lawyer',"Lawer"])]  # filter specific values
...
users.sort_index(by=['age','occupation'])  # sort by multiple columns
